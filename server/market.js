import { utcDay, utcMinute, utcParse } from "d3";

const randomize = (stock, i) =>
  Object.assign(stock, {
    waveFreq: 0.5 + 1.5 * Math.random(),
    waveShift: (0.5 + 1.5) * Math.random(), // + (i / stocks.length)
  });
...
class Market {
  constructor(startAt = 0) {
    const timestamp = (this.timestamp = utcMinute
      .offset(utcDay.floor(Date.now()), DAY_START * 60)
      .valueOf()); // market opens at 9:30am
  ...
  startNewDay() {
    const timestamp = (this.timestamp = utcMinute
      .offset(utcDay.ceil(this.timestamp), DAY_START * 60)
      .valueOf()); // market re-opens at 9:30am the next day
    stocks.forEach(randomize);
  ...
  generateMarketEvents() {
    ...
    const timestamp = (this.timestamp = utcMinute
      .offset(this.timestamp, 1)
      .valueOf());
    const changes = stocks.map(({ ticker, waveFreq, waveShift }) => {
      if (Math.random() > CHANGE_ODDS) return { ticker, change: 0 };
      else {
        const f =
          ((this.history.length / stocks.length) % samplesPerDay) /
          samplesPerDay; // fraction of trading day
